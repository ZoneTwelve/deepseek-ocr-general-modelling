...
class DeepseekOCRForCausalLM(DeepseekV2ForCausalLM):
    ...
    def infer(self, tokenizer, prompt='', image_file='', output_path = '', base_size=1024, image_size=640, crop_mode=True, test_compress=False, save_results=False, eval_mode=False):
        ...
        # Based on the device select data type
        ...
        # Decide whether to enable autocast depending on the device type
            ...
            if use_autocast:
                ...
                with torch.no_grad():
                    output_ids = self.generate(
                        ...
                        streamer=streamer,
                        max_new_tokens=4096,  # Reduce maximum new token
                        min_new_tokens=1,
                        no_repeat_ngram_size = 20,
            ...
            if use_autocast:
                ...
                with torch.no_grad():
                    output_ids = self.generate(
                        ...
                        eos_token_id=tokenizer.eos_token_id,
                        pad_token_id=tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id,
                        max_new_tokens=4096,  # Reduce max new tokens
                        min_new_tokens=1,
                        no_repeat_ngram_size = 35,
                        repetition_penalty=1.2,  # Line Added
                        use_cache = True
                        )
